...
```bash
L=/usr/local/bin/conoha && curl -sL https://github.com/hironobu-s/conoha-vps/releases/download/v20150223.1/conoha-osx.amd64.gz | zcat > $L && chmod +x $L
```
...
```bash
L=/usr/local/bin/conoha && curl -sL https://github.com/hironobu-s/conoha-vps/releases/download/v20150223.1/conoha-linux.amd64.gz | zcat > $L && chmod +x $L
```
...
[ZIPファイル](https://github.com/hironobu-s/conoha-vps/releases/download/v20150223.1/conoha.amd64.zip)をダウンロードして、適当なフォルダに展開します。
